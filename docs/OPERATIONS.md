...
- **公开社媒发现**每小时:Brave 站点限定查询补充公开 TK / IG / FB / YouTube / X / Reddit / 论坛 / 评论页；另有 WHIO、WPRI、KENS、KARE、Boston 25 五个已知新闻媒体 Facebook 账号各自每天一次的定向索引查询。两者合计约 29 次/天，保持免费配额；每个新定向源的首次成功采集均静默入库，避免把索引存量当作实时新舆情推送。
...
- **社媒母帖补录**:复核页的「补录公开社媒母帖」仅接收公开链接和操作人可见的标题/摘要；不登录平台、不抓正文或评论。补录记录标记为 `manual_public_link / manual_supplied`，静默进入证据库和传播簇，不重复发送钉钉。
- **源健康与发现量**:scheduler 每 30 分钟检查抓取源。**P0 源**(Google News RSS / AboutLawsuits)失联或连续失败时推钉钉并 @ 负责人；持牌或公开社媒源连续失败时也会发送不 @ 的健康提醒。看板同时记录每次巡检的返回、有效、新/更新入库、过滤和重复数；“状态正常”只代表调用成功，不代表该平台没有讨论。
...
高敏车道只用免费、无限额的 RSS/CPSC,所以商业 API 配额打满也不会拖垮时效。Brave 的常规广泛查询约 24 次/天，5 个媒体账号定向查询共约 5 次/天；GNews 独立按每小时运行。是否升级付费档,以试运行期间「是否有重要信号只在每小时商业源出现、被 15 分钟免费源漏掉」为依据再决定。
...
除 CLI 外,FastAPI 应用提供网页复核入口:`GET /review`。默认「待处置队列」只显示仍需要人工判断的红/黄事件；「全量证据库」可筛选全部记录、已归档内容和已排除误报。每条带「确认 / 误报 / 静音7天」按钮,点一下即按事件标记(等价于 `review mark`)。人工结论优先于算法初检：已确认、误报或仍在有效期的静音事件不再显示“需人工核实 / 证据待复核”待办标签；原始自动判断保留在“自动初检留档”中供审计。

- **误报统计口径**:「标记误报」会永久抑制该事件的后续告警，并立即从分析看板、趋势、日报与运营统计中剔除；原文、链接和人工结论保留在全量证据库与 CSV（`statistical_inclusion=已排除误报`）供审计。
- **人工介入分流**:每条记录独立标记为「待分流 / 仅监测 / 人工查看评论 / 建议回应 / 升级 PR/法务 / 已归档」。新闻稿转载默认是「仅监测」，不是待回应；系统不发布、回复、点赞或删除任何外部平台内容。
- **批量复核**:复核页可勾选当前页多个事件，并批量执行「确认并跟进 / 标记误报 / 静音 7 天」。每个事件仍独立写入复核结论、告警确认与审计记录；单次最多 100 个，提交前必须二次确认。
...
- **分析看板** `GET /review/analysis?days=30`:发布量与发现工作量双走线、传播链接/独立传播簇/新增实质信号三层口径、平台趋势、跨平台扩散、关键词/议题、传播角色和覆盖/延迟矩阵；旧地址 `/dashboard` 保持兼容。传播结构与议题归纳均可点击进入统一明细页，按传播角色或议题查看原文证据。已标记误报会明确显示剔除数，但不会进入任何看板指标、趋势、明细或日报。“议题归纳”以可解释的展示规则区分“召回/监管传播”“法律行动”“事故/伤害陈述”等：只有明确法律动作才进入“法律行动”，召回报道中的既有伤亡事实不等同于诉讼。“未识别新增实质信号”只是自动规则结果，不替代人工核实。
- **传播数据明细** `GET /review/analysis/details?metric=clusters&days=30`:由看板顶部指标卡与议题行进入统一明细页；支持 `links`、`clusters`、`syndicated`、`substantive`、`parent_posts` 五类标签，并可按 `topic` 过滤，继承平台/主题/时间筛选，列表每页 100 项。传播簇可展开查看全部成员，并跳转公开原文或对应复核记录。“进入复核页”会直接打开全量证据库中指定的独立记录；完成事件复核或逐条处置后会返回原明细、看板或当前筛选页，不会跳回空的待处置队列。
