...
def _render_incident(
    ...
    if human_closed:
        ...
        audit_note = (
            '<details class="review-correction audit-note"><summary>查看自动初检留档</summary>'
...
def _public_social_intake_form(csrf_token: str, return_to: str = "") -> str:
    """A manual fallback that never attempts to read a platform's comments."""
    return (
        '<details class="public-social-intake"><summary>补录公开社媒母帖</summary>'
        '<p>用于搜索未收录的公开 Facebook、Instagram、TikTok、YouTube、X 或 Reddit 母帖。'
        '仅保留链接和您可见的标题/摘要，不抓平台正文或评论；补录不会重复发送钉钉提醒。</p>'
        '<form method="post" action="/review/public-social-intake" class="public-social-intake-form">'
        f'<input type="hidden" name="csrf" value="{escape(csrf_token, quote=True)}">'
        f'<input type="hidden" name="return_to" value="{escape(return_to, quote=True)}">'
        '<label>公开母帖链接<input name="source_url" type="url" placeholder="https://www.facebook.com/..." required></label>'
        '<label>原帖可见标题（可选）<input name="title" maxlength="500" placeholder="复制页面可见标题即可"></label>'
        '<label class="wide">可见摘要/上下文（可选）<textarea name="evidence_excerpt" rows="2" maxlength="4000" placeholder="仅粘贴页面上已经可见的文字；不要转录评论区个人信息"></textarea></label>'
        '<button type="submit">补录并归入传播簇</button></form></details>'
    )
...
def render_review_page(
    ...
    public_social_intake = _public_social_intake_form(csrf_token, return_to)
    return (
        ...
        '.audit-note{margin:0 0 14px;color:var(--muted);font-size:.82rem}.audit-note p{margin:9px 0 0;color:var(--muted)}'
        ...
        f'{public_social_intake}{filter_form}'
